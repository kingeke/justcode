...
export const tools: Item[] = [
  { name: 'read_file', description: 'Read a file, with line windows for large files.' },
  { name: 'write_file', description: 'Create a new file or overwrite an existing one.' },
  { name: 'edit_file', description: 'Make exact find-and-replace edits inside a file.' },
  { name: 'apply_patch', description: 'Apply a multi-hunk patch across one or more files.' },
  { name: 'bash', description: 'Run shell commands in your working directory.' },
  ...
  { name: 'todowrite', description: 'Track a task list to stay organized on multi-step work.' },
  { name: 'question', description: 'Ask you a clarifying question mid-task when it matters.' },
  { name: 'present_plan', description: 'Propose an implementation plan before touching code (Plan mode).' },
  { name: 'view_history', description: 'Look back through the conversation and session history.' },
  { name: 'lazy_load_tools', description: 'Load extra tools on demand to keep requests lean.' },
];

/** The chat modes (plus user-defined custom modes). */
export const modes: Item[] = [
  { name: 'Build', description: 'Implement changes with the full toolset — the default working mode.' },
  { name: 'Ask', description: 'Read-only Q&A about your codebase; explains without editing.' },
  { name: 'Plan', description: 'Produce a reviewable plan first, then implement it on approval.' },
  { name: 'Custom', description: 'Define your own mode with its own system prompt via /mode.' },
];
...
export const commands: Item[] = [
  ...
  { name: '/refresh-models', description: 'Re-fetch every provider model list, bypassing the cache.' },
  { name: '/sessions', description: 'Browse saved sessions and resume one.' },
  { name: '/new-session', description: 'Start a fresh conversation in a new session.' },
  ...
  { name: '/thinking', description: 'Toggle whether model reasoning is shown.' },
  { name: '/reasoning', description: 'Choose reasoning effort for the current model.' },
  { name: '/auto-approve', description: 'Toggle auto-approving all tool actions without confirmation.' },
  { name: '/local-model-refresh', description: 'Toggle always refreshing local models (off uses a daily cache).' },
  { name: '/toggle-lazy-tool-loading', description: 'Toggle lazy tool loading (off sends all tools by default).' },
  { name: '/expand-tools', description: 'Toggle showing full tool input/output inline by default.' },
  { name: '/manage-tools', description: 'Turn individual tools on or off.' },
  { name: '/mode', description: 'Switch the chat mode or create a custom one.' },
  { name: '/implement', description: 'Switch to Build mode and implement the latest plan.' },
  { name: '/edit-plan', description: 'Save the latest plan to a file to edit before implementing.' },
  { name: '/collapse-responses', description: 'Toggle hiding model responses to scan just your messages.' },
  { name: '/configure-mcp-servers', description: 'Open mcp.json to add or edit MCP servers.' },
  { name: '/read-limit', description: 'Set how many lines of a file the model reads at once.' },
  { name: '/context-window', description: 'Set how many recent context items are sent to the model.' },
  { name: '/config', description: 'Open the config file in your editor.' },
  { name: '/reset', description: 'Reset app defaults and clear connected providers and models.' },
];
...
export const providers: Item[] = [
  ...
  { name: 'OpenRouter', description: 'One key, hundreds of models across providers.' },
  { name: 'Alibaba (Qwen)', description: "Qwen via Alibaba Cloud's OpenAI-compatible API." },
  { name: 'Ollama', description: 'Run open models locally, offline.' },
  { name: 'LM Studio', description: 'Local models through LM Studio.' },
  { name: 'OpenAI-compatible', description: 'Point at any custom OpenAI-compatible endpoint.' },
];

/** Install one-liners per channel. */
export const installCommands: { label: string; command: string }[] = [
  { label: 'curl', command: 'curl -fsSL https://raw.githubusercontent.com/kingeke/justcode/main/scripts/install.sh | sh' },
  { label: 'npm', command: 'npm install -g justcode' },
  { label: 'brew', command: 'brew tap kingeke/justcode && brew install justcode' },
];
